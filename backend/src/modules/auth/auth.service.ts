...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<AuthResponseDto> {
    ...
    if (existingUsername) {
      this.logger.warn(`Registration failed: username ${dto.username} already exists`);
  ...
  async login(dto: LoginDto): Promise<AuthResponseDto> {
    ...
    if (user.accessBlocked) {
      this.logger.warn(`Login failed: account disabled for ${dto.email}`);
  ...
  async logout(refreshToken: string): Promise<LogoutResponseDto> {
    ...
    this.winstonLoggerService.log(`Refresh token blacklisted for user: ${payload.sub}`, 'AuthService');
  ...
  async oauthLogin(provider: string, oauthUserInfo: OAuthUserInfoDto): Promise<AuthResponseDto> {
    ...
    this.winstonLoggerService.log(`OAuth login successful for user: ${user.id} via ${provider}`, 'AuthService');
